...
# Initialize TTS engine once and reuse it
engine = pyttsx3.init()
...
def handle_text_command(command: str):
    logging.info(f"📥 Text Command: {command}")
    logging.info("🤖 Sending command to agent...")
    response = executor.invoke({"input": command})
    content = response["output"]
    logging.info(f"✅ Agent responded: {content}")
...
if __name__ == "__main__":
    mode = input("Type 'voice' for voice commands or 'text' for text commands: ").strip().lower()
    if mode == "voice":
        write()
    elif mode == "text":
        while True:
        ...
        print("Invalid mode. Please restart and choose 'voice' or 'text'.")
